...
using static AwAVR.HierarchyPlus.ContentContainer;
...
namespace AwAVR.HierarchyPlus
{
    ...
    internal class SavedSettings
    {
        ...
        private static void Load()
        {
            ...
            catch (Exception ex)
            {
                HierarchyPlus.CustomLog($"There was an error loading settings. Settings have been reset.\n\n{ex}",
                    HierarchyPlus.CustomLogType.Warning);
        ...
        {
            if (EditorUtility.DisplayDialog("Clearing Settings", "Are you sure you want to clear the settings?",
                    "Clear", "Cancel")) Clear();
        }
        ...
        internal class SavedBool : SavedValue
        {
            ...
            internal void DrawField(string label, string tooltip = "", GUIStyle style = null,
                params GUILayoutOption[] options)
                => DrawField(new GUIContent(label), tooltip, style, options);


            internal void DrawField(GUIContent label, string tooltip = "", GUIStyle style = null,
                params GUILayoutOption[] options)
            {
                if (!string.IsNullOrWhiteSpace(tooltip)) label.tooltip = tooltip;
            ...
            internal void DrawToggle(string activeLabel, string inactiveLabel = null, GUIStyle style = null,
                Color? activeColor = null, Color? inactiveColor = null, params GUILayoutOption[] options)
                => DrawToggle(string.IsNullOrEmpty(activeLabel) ? GUIContent.none : new GUIContent(activeLabel),
                    string.IsNullOrEmpty(inactiveLabel) ? GUIContent.none : new GUIContent(inactiveLabel), style,
                    activeColor, inactiveColor, options);

            internal void DrawToggle(GUIContent activeLabel, GUIContent inactiveLabel = null, GUIStyle style = null,
                Color? activeColor = null, Color? inactiveColor = null, params GUILayoutOption[] options)
            {
                ...
                value = GUILayout.Toggle(value, value || inactiveLabel == null ? activeLabel : inactiveLabel,
                    style == null ? GUI.skin.button : style, options);
        ...
        internal class SavedColor : SavedValue
        {
            ...
            internal void DrawField(GUIContent label, bool drawReset = true, params GUILayoutOption[] options)
            {
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    if (!drawReset) return;
                    if (GUILayout.Button(Content.resetIcon, Styles.labelButton, GUILayout.Width(18),
                            GUILayout.Height(18)))
                        Reset();
